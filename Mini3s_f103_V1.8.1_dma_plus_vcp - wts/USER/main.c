...
sfConfig_t sfConfig[4] ={
                    //mode 1 - S1: 2 off, 3 off
										{
											(28), //slot period ms
											(2),//(10),   //thus 10 slots - thus 280ms superframe means 3.57 Hz location rate (10 slots are needed as AtoA ranging takes 30+ ms)
											(2*28),//(10*28), //superframe period
											(2*28),//(10*28), //poll sleep delay
											(20000)////response delay time 单位：us
										},
                    //mode 2 - S1: 2 on, 3 off
                    {
                    	(10),   // slot period ms
                    	(2),//(10),   // number of slots (only 10 are used) - thus 100 ms superframe means 10 Hz location rate
											(2*10),//(10*10), // superframe period (100 ms - gives 10 Hz)
											(2*10),//(10*10), // poll sleep delay (tag sleep time, usually = superframe period)
											(2500)
                    },
                    //mode 3 - S1: 2 off, 3 on
                    {
...
void addressconfigure(uint8 s1switch, uint8 mode)
{
    ...
    instance_anchaddr = (((s1switch & SWS1_A1A_MODE) << 2) + (s1switch & SWS1_A2A_MODE) + ((s1switch & SWS1_A3A_MODE) >> 2)) >> 4;
		//instance_anchaddr = 0 0 0 0 0 N3 N2 N1
    if(mode == ANCHOR)
    {
...
uint32 inittestapplication(uint8 s1switch)
{
    ...
    addressconfigure(s1switch, instance_mode) ; // address = 0 0 0 0 0 N3 N2 N1
...
u8 Transfer_Byte(u8 value)//value = 1 Speed Channel Mode N3 N2 N1 0 //N3N2N1模块地址
{
	value = (value & 0xaa)>> 1 | (value & 0x55)<<1;
	value = (value & 0xcc)>> 2 | (value & 0x33)<<2;
	value = (value & 0xf0)>> 4 | (value & 0x0f)<<4;
	return value;//Transfer = 0 N1 N2 N3 Mode Channel Speed 0
}
u8 Check_Switch_Status(u16 value)//value = (AT+SW=)1XXXXXX0
{
	u8 temp = 0x00;
	if( ((u8)(value>>8)) != 0xAA )
		return 0;
...
{ 
  ...
  //delay_init();	    	 //延时函数初始化
	...
	temp_switch = Check_Switch_Status(sys_para.device_switch);//存AT命令进行虚拟拨码开关设置
	...
	s1switch = Transfer_Byte(temp_switch & 0x7f);//Transfer = 0 N1 N2 N3 Mode Chanel Speed 0
	printf("s1switch =%02x\r\n",s1switch);
...
//		        }
            ...
            if(rx == TOF_REPORT_T2A)
            {
            	...
            	n = sprintf((char*)&SendBuff[0], "mc %02x %08d %08d %08d %08d %04x %02x %08x %c%d:%d\r\n",
...
//														(instance_mode == TAG)?'t':'a', taddr, aaddr);
            	...
            	{
            		n += sprintf((char*)&SendBuff[n], "mr %02x %08d %08d %08d %08d %04x %02x %04x%04x %c%d:%d\r\n",
