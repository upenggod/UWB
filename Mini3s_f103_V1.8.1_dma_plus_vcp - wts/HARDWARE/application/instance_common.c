...
{
		int x = instance_data[0].newRange;//获得newRange状态
...
void inst_processrxtimeout(instance_data_t *inst)
{
    ...
    if(inst->mode == ANCHOR) //ANCHOR did not receive the final - wait for next poll
    {
  ...
  if(rxd->event == DWT_SIG_RX_OKAY)
	{
		...
		if(((rxd->fctrl[0] == 0x41) || (rxd->fctrl[0] == 0x61))//两个字节的帧控制字节对于TREK应用是恒定的（0X41、0X88）
				&&
				((rxd->fctrl[1] & 0xCC) == 0x88)) //short address
		{

			fcode_index = FRAME_CRTL_AND_ADDRESS_S; //function code is in first byte after source address 9Byte
			srcAddr_index = FRAME_CTRLP + ADDR_BYTE_SIZE_S;//5+2
